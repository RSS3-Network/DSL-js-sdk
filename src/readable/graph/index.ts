...
export function activityToDot(activity: Activity, action: components['schemas']['Action']): Dot {
  const res = {
    from: action.from,
    to: action.to,
    verb: action.type,
...
function formatToken(m: components['schemas']['TokenMetadata'] | null | undefined) {
  if (!m)
    return {
      type: 'token',
      content: '0 eth',
...
function formatExchange(m: components['schemas']['ExchangeSwap']) {
  ...
  const content = [from, to]
    .map((item) => item.filter((t) => t.type !== 'symbolImage').map((t) => t.content))
    .join(' to ')
  return {
    type: 'exchange',
    content: content,
...
function formatLiquidity(m: components['schemas']['ExchangeLiquidity']) {
  ...
  return {
    type: 'liquidity',
    content: tokens.map((item) => item.map((t) => t.content).join(' ')).join(' '),
  }
}

function formatLoan(m: components['schemas']['TokenMetadata'] | null | undefined) {
...
function formatMetaverse(m: components['schemas']['MetaverseTransfer'] | components['schemas']['MetaverseTrade']) {
  return {
    type: 'metaverse',
